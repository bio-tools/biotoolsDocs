...
5. We run `hangouts <http://biotools.readthedocs.io/en/latest/hangouts.html>`_ (coordination meetings) as required - mostly for technical people routinely involved with *bio.tools* curation or software development.  To suggest or join these calls email `Jon <mailto:jison@bioinformatics.dtu.dk>`_.
6. Dive in at the deep end!  There are no end of ongoing sub-projects and tasks to get involved with, see GitHub (at above links) or email `Registry Support <mailto:registry-support@elixir-dk.org>`_ in the 1st instance to get orientated.
